...
namespace btnet
{
	...
	public class BugList {
            ...
            )
        {
            ...
            Response.Write("<td align=right valign=top><span class=smallnote>Нажмите и удерживайте Ctrl для установки \"NOT\" в фильтре: \"NOT project 1\"</span></table>");
            ...
            foreach (DataColumn dc in dv.Table.Columns)
            {
                ...
                else
                {
                    ...
                    else if (dc.ColumnName.ToLower().IndexOf("desc") == 0)
                    {
                        ...
                        Response.Write("описание");//(dc.ColumnName);
                    }
                    ...
                    {
                        search_source_column = db_column_count;
                        Response.Write("проект");
                    }
                    else if (dc.ColumnName == "organization")
                    {
                        search_source_column = db_column_count;
                        Response.Write("организация");
                    }
                    else if (dc.ColumnName == "category")
                    {
                        search_source_column = db_column_count;
                        Response.Write("категория");
                    }
                    else if (dc.ColumnName == "reported by")
                    {
                        search_source_column = db_column_count;
                        Response.Write("создано");
                    }
                    else if (dc.ColumnName == "priority")
                    {
                        search_source_column = db_column_count;
                        Response.Write("приоритет");
                    }
                    else if (dc.ColumnName == "assigned to")
                    {
                        search_source_column = db_column_count;
                        Response.Write("ассоциировано с");
                    }
                    else if (dc.ColumnName == "status")
                    {
                        search_source_column = db_column_count;
                        Response.Write("статус");
                    }
                    else if (dc.ColumnName == "last updated by")
                    {
                        search_source_column = db_column_count;
                        Response.Write("последнее обновление");
                    }
                    else if (dc.ColumnName == "last updated on")
                    {
                        search_source_column = db_column_count;
                        Response.Write("дата последнего обновления");
                    }
                    else
                    {
