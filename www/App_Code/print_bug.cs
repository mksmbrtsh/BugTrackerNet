...
namespace btnet
{
	...
	public class PrintBug {
            ...
            bool internal_posts)
		{
			...
			Response.Write ("Название:&nbsp;<a href="
				+ btnet.Util.get_setting("AbsoluteUrlPrefix","http://127.0.0.1/")
				+ "edit_bug.aspx?id="
				+ string_bugid
				+ ">"
            ...
            if (security.user.status_field_permission_level > 0)
            	Response.Write("\n<tr><td>Статус<td>" + dr["status_name"] + "&nbsp;");
            ...
            bool this_external_user)
		{
			...
			*/
			...
			else if (type == "comment" )
			{
				...
				Response.Write ("<span class=pst>комментарий <a name=" + Convert.ToString(post_id) +  "></a>" + Convert.ToString(post_id) + " опубликован ");
				Response.Write (format_email_username(
					write_links,
					bugid,
                    permission_level,
				...
				else if (type == "file")
				{
					...
					&& permission_level == Security.PERMISSION_ALL))
					{
						...
						Response.Write (" href=edit_attachment.aspx?id="
							+ string_post_id + "&bug_id=" + string_bug_id);
						Response.Write (">редактировать</a>");
