...
<td nowrap valign=middle>
    <form style='margin: 0px; padding: 0px;' action=edit_bug.aspx method=get>
        <input class=menubtn type=submit value='перейти к ID'>
...
<table border=0 width=100% cellpadding=0 cellspacing=0 class=menubar><tr>");
            ...
            write_menu_item(Response, this_link, "главная", "bugs.aspx");
            
            if (user.can_search)
            {
            	write_menu_item(Response, this_link, "поиск", "search.aspx");
            }
            ...
            if (!user.is_guest)
            {
                write_menu_item(Response, this_link, "запросы", "queries.aspx");
            }

            if (user.is_admin || user.can_use_reports || user.can_edit_reports)
            {
                write_menu_item(Response, this_link, "отчеты", "reports.aspx");
            }
            ...
            if (user.is_admin)
            {
                write_menu_item(Response, this_link, "администрирование", "admin.aspx");
            }
            else if (user.is_project_admin)
            {
                write_menu_item(Response, this_link, "пользователи", "users.aspx");
            }
...
<td nowrap valign=middle>
    <form style='margin: 0px; padding: 0px;' action=search_text.aspx method=get onsubmit='return on_submit_search()'>
        <input class=menubtn type=submit value='поиск текста'>
        <input class=menuinput  id=lucene_input size=24 type=text class=txt
        value='" + query.Replace("'","") + @"' name=query accesskey=s>
        <a href=lucene_syntax.html target=_blank style='font-size: 7pt;'>дополнительно</a>
    </form>
</td>";
			...
			{
				Response.Write("<span class=smallnote>вы вошли как<br>");
			}
            ...
            if (auth_method == "plain")
            {
                if (user.is_guest && Util.get_setting("AllowGuestWithoutLogin","0") == "1")
                {
                	write_menu_item(Response, this_link, "вход", "default.aspx");
				}
				else
				{
					write_menu_item(Response, this_link, "выход", "logoff.aspx");
				}
            }
            ...
            if (!user.is_guest)
            {
                write_menu_item(Response, this_link, "настройки", "edit_self.aspx");
            }


            Response.Write("<td valign=middle align=left'>");
            Response.Write("<a target=_blank href=about.html><span class='menu_item' style='margin-left:3px;'>о программе</span></a></td>");
            Response.Write("<td nowrap valign=middle>");
            Response.Write("<a target=_blank href=http://ifdefined.com/README.html><span class='menu_item' style='margin-left:3px;'>помощь</span></a></td>");
