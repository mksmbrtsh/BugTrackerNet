    ...
    public class SortableHtmlTable
    {
            ...
            HttpResponse r, bool write_column_headings_as_links)
        {
            ...
            if (write_column_headings_as_links)
            {
                ...
                r.Write("<div class=click_to_sort>Нажмите на название столбца для сортировки</div>\n");
            }
            ...
            bool write_column_headings_as_links)
        {
            ...
            foreach (DataColumn dc in ds.Tables[0].Columns)
            {
                ...
                && db_column_count == (ds.Tables[0].Columns.Count - 1))
                {
                    if (edit_url != "")
                    {
                        r.Write("<td class=datah valign=bottom>редактирование</td>");
                    }
                    if (delete_url != "")
                    {
                        r.Write("<td class=datah valign=bottom>удаление</td>");
                    }
                    ...
                    && i == (ds.Tables[0].Columns.Count - 1))
                    {
                        if (edit_url != "")
                        {
                            r.Write("<td class=datad><a href="
                                + edit_url + dr[ds.Tables[0].Columns.Count - 1] + ">редактирование</a></td>");
                        }
                        if (delete_url != "")
                        {
                            r.Write("<td class=datad><a href="
                                + delete_url + dr[ds.Tables[0].Columns.Count - 1] + ">удаление</a></td>");
                        }
                    }
                    else
                    {
