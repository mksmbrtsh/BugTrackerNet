...
namespace btnet
{
	...
	public class Util {
		...
		public static string is_valid_decimal(string name, string val, int left_of_decimal, int right_of_decimal)
		{
			...
			{
				return name + " не допустимый формат для десятичных чисел";
			}
			...
			if (vals.Length > 0)
			{
				...
				{
					return name  +  " слишком много точек слева от разделителя дробной части";
				}
			}
			else if (vals.Length > 1)
			{
				...
				{
					return name + " слишком много точек справа от разделителя дробной части";
				}
			}
		...
		public static string validate_dropdown_values(string vals)
		{
			...
			|| vals.Contains("\t"))
			{
				return "Специальные символы, такие как <, >, или кавычки не разрешены.";
			}
			else
			{
				return "";
		...
		public static string how_long_ago(TimeSpan ts)
        {
            ...
            if (ts.Days > 0)
            {
                if (ts.Days == 1)
                {
                    if (ts.Hours > 2)
                    {
                        String s = "1 день и ";
                        String ss;
                        if (ts.Hours == 1 || (ts.Hours % 10 == 1 && ts.Hours != 11))
                        {
                            ss = ts.Hours + " час назад";
                        }
                        else if ((ts.Hours % 10 == 2 || ts.Hours % 10 == 3 || ts.Hours % 10 == 4) && ts.Hours != 12 && ts.Hours != 13 && ts.Hours != 14)
                        {
                            ss = ts.Hours + " часа назад";
                        }
                        else
                            ss = ts.Hours + " часов назад";
                        return s + ss;
                    }
                    else
                    {
                ...
                {
                    if (ts.Days == 1 || (ts.Days % 10 == 1 && ts.Days != 11))
                    {
                        return ts.Days + " день назад";
                    }
                    else if ((ts.Days % 10 == 2 || ts.Days % 10 == 3 || ts.Days % 10 == 4) && ts.Days != 12 && ts.Days != 13 && ts.Days != 14)
                    {
                        return ts.Days + " дня назад";
                    }
                    else
            ...
            else if (ts.Hours > 0)
            {
                if (ts.Hours == 1)
                {
                    if (ts.Minutes > 5)
                    {
                        String s = "1 час и ";
                        String ss;
                        if (ts.Minutes == 1 || (ts.Minutes % 10 == 1 && ts.Minutes != 11))
                        {
                            ss = ts.Minutes + " минута назад";
                        }
                        else if ((ts.Minutes % 10 == 2 || ts.Minutes % 10 == 3 || ts.Minutes % 10 == 4) && ts.Minutes != 12 && ts.Minutes != 13 && ts.Minutes != 14)
                        {
                            ss = ts.Minutes + " минуты назад";
                        }
                        else
                            ss = ts.Minutes + " минут назад";
                        return s + ss;
                    }
                    else
                    {
            ...
            {
                String ss;
                if (ts.Seconds == 1 || (ts.Seconds % 10 == 1 && ts.Seconds != 11))
                {
                    ss = ts.Seconds + " секунду назад";
                }
                else if ((ts.Seconds % 10 == 2 || ts.Seconds % 10 == 3 || ts.Seconds % 10 == 4) && ts.Seconds != 12 && ts.Seconds != 13 && ts.Seconds != 14)
                {
                    ss = ts.Seconds + " секунды назад";
                }
                else
                    ss = ts.Seconds + " секунд назад";
                return ss;
            }
