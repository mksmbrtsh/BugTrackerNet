...
void Page_Load(Object sender, EventArgs e)
{
	...
	if (string_bugid == null || string_bugid == "0" || (string_bugid != "0" && clone_ignore_bugid.Value == "1"))
	{
        // New
		id = 0;
        bugid_label.InnerHtml = "Заголовок:&nbsp;";
...
void prepare_a_bunch_of_links_for_update()
{
    ...
    string toggle_images_link = "<a href='javascript:toggle_images2("
        ...
        + (images_inline ? "скрывать" : "показывать")
    ...
    string toggle_history_link = "<a href='javascript:toggle_history2("
        ...
        + (history_inline ? "скрыть" : "показать")
        + " изменение истории"
        + "</span></a>";
    ...
    if (permission_level == Security.PERMISSION_ALL)
    {
        string clone_link = "<a class=warn href=\"javascript:clone()\" "
            + " title='Создать копию этой записи'><img src=paste_plain.png border=0 align=top>&nbsp;копировать запись</a>";
    ...
    if (permission_level != Security.PERMISSION_READONLY)
    {
        string attachment_link = "<img src=attach.gif align=top>&nbsp;<a href=\"javascript:open_popup_window('add_attachment.aspx','add attachment ',"
            + Convert.ToString(id)
            + ",600,300)\" title='Attach an image, document, or other file to this item'>добавить вложение</a>";
    ...
    if (!security.user.is_guest)
    {
        if (permission_level != Security.PERMISSION_READONLY)
        {
            string send_email_link = "<a href='javascript:send_email("
                + Convert.ToString(id)
                + ")' title='Send an email about this item'><img src=email_edit.png border=0 align=top>&nbsp;отправить на почту</a>";
    ...
    if (permission_level != Security.PERMISSION_READONLY)
    {
        string subscribers_link = "<a target=_blank href=view_subscribers.aspx?id="
            + Convert.ToString(id)
            + " title='Пользователи, которые будут получать оповещение по почте об этой записи'><img src=telephone_edit.png border=0 align=top>&nbsp;подписчики</a>";
    ...
    print.InnerHtml = "<a target=_blank href=print_bug.aspx?id="
        + Convert.ToString(id)
        + " title='Отобразить записиь в формате для печати'><img src=printer.png border=0 align=top>&nbsp;печать</a>";
    ...
    if (!security.user.is_guest)
    {
        ...
        || security.user.can_merge_bugs)
        {
            string merge_bug_link = "<a href=merge_bug.aspx?id="
                + Convert.ToString(id)
                + " title='Соединить запись с другой'><img src=database_refresh.png border=0 align=top>&nbsp;соединить</a>";
    ...
    if (!security.user.is_guest)
    {
        ...
        || security.user.can_delete_bug)
        {
            string delete_bug_link = "<a href=delete_bug.aspx?id="
                + Convert.ToString(id)
                + " title='Удалить эту запись'><img src=delete.png border=0 align=top>&nbsp;удалить</a>";
...
void format_subcribe_cancel_link()
{
	...
	if (notification_email_enabled)
	{
		...
		else
		{
			...
			string subscription_link = "<a id='notifications' title='Подписка или отписка от оповещений по почте об этой записи'"
			...
			if (subscribed > 0)
			{
				subscription_link += "перестать получать оповещение</span></a>";
			}
			else
			{
				subscription_link += "получать оповещение</span></a>";
			}
		...
		if (this_bug_found)
		{
			if (prev_bug != 0)
			{
				prev_next_link =
					...
					+ "><img src=arrow_up.png border=0 align=top>назад</a>";
			}
			else
			{
				prev_next_link = "&nbsp;&nbsp;&nbsp;&nbsp;<span class=gray_link>назад</span>";
			}
			...
			if (next_bug != 0)
			{
				prev_next_link +=
					...
					+ ">вперед<img src=arrow_down.png border=0 align=top></a>";

			}
			else
			{
				prev_next_link += "&nbsp;&nbsp;&nbsp;&nbsp;<span class=gray_link>вперед</span>";
			}
...
<a class=warn href="edit_bug.aspx?id=0"><img src=add.png border=0 align=top>&nbsp;добавить</a>
&nbsp;&nbsp;&nbsp;&nbsp;
<% } %>
...
<form class=frm runat="server">
	...
	<tr>
		...
		<td nowrap align=right id="presets" >Предварительные настройки:
			<a title="Используйте ранее сохраненные настройки для полей..."
				href="javascript:get_presets()">использовать</a>
			&nbsp;/&nbsp;
			<a title="Сохранить текущие настройки для проекта, категория, приоритет и т.д., так что вы можете повторно использовать позже."
				href="javascript:set_presets()">сохранить</a>
		<% } %>
	...
	<tr id="row1">
		<td nowrap>
			<span class=lbl id="project_label" runat="server">Проект:&nbsp;</span>
		<td nowrap>
	...
	<tr id="row2">
		<td nowrap>
			<span class=lbl id="org_label" runat="server">Организация:&nbsp;</span>
		<td nowrap>
	...
	<tr id="row3">
		<td nowrap>
			<span class=lbl id="category_label" runat="server">Категория:&nbsp;</span>
		<td nowrap>
	...
	<tr id="row4">
		<td nowrap>
			<span class=lbl id="priority_label" runat="server">Приоритет:&nbsp;</span>
		<td nowrap>
	...
	<tr id="row5">
		<td nowrap>
			<span class=lbl id="assigned_to_label" runat="server">Ассоциировано с:&nbsp;</span>
		<td nowrap>
	...
	<tr id="row6">
		<td nowrap>
			<span class=lbl id="status_label" runat="server">Статус:&nbsp;</span>
		<td nowrap>
...
%>
	...
	<tr><td nowrap>
		...
		&nbsp;
		<span id="comment_label" runat="server">Комментарий:</span>
...
<% 
		...
		{		
			...
			Response.Write ("Введите \"" 
				+ btnet.Util.get_setting("BugLinkMarker","bugid#") 
				+"999\" в комментарии для ссылки к id 999");
		} 
%>		
		</span>
		<br>
