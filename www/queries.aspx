...
void Page_Load(Object sender, EventArgs e)
{
	...
	else
	{
		...
		sql =  @"select
			qu_desc [запрос],
			'<a href=bugs.aspx?qu_id=' + convert(varchar,qu_id) + '>view list</a>' [просмотр списка],
			...
			'<a href=edit_query.aspx?id=' + convert(varchar,qu_id) + '>rename</a>' [переименовать],
...
<html>
<head>
<title id="titl" runat="server">btnet запросы</title>
...
<% if (security.user.is_admin || security.user.can_edit_sql) { %>
	<table border=0 width=80%><tr>
		<td align=left valign=top>
			<a href=edit_query.aspx>добавить новый запрос</a>
		<td align=right valign=top>
			<form runat="server">
				<span class=lbl>показывать личные запросы везде:</span>
...
{
	Response.Write ("Нет запросов в базе данных.");
}
